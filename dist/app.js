...
app.get("/aiChallenge", (req, res) => {
    getCompletion().then((response) => {
...
// }
let dbClient;
app.post('/postMedia', upload.single("video"), (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    dbClient = client;
...
app.delete('/deleteMedia/:id', (req, res) => {
    MongoClient.connect(uri, (err, client) => {
        ...
        const bucket = new GridFSBucket(client.db("BeCrazy"), { bucketName: 'videos' });
        collection.deleteOne({ videoId: ObjectId(req.params.id) }, (err, result) => {
            if (err) {
                console.log(err);
                return;
            }
        });
        bucket.delete(ObjectId(req.params.id), (err) => {
            if (err) {
                console.log(err);
...
app.get('/getAllMedia', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
...
app.get('/top10media', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const result = yield collectionAllMedia.find({ created: { $regex: `^${date}` } }).sort({ nbLikes: -1 }).limit(10).toArray();
        res.send(result);
    }
    catch (err) {
        res.status(500).send(err);
...
app.get('/searchUser/:username', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const result = yield collectionUsers.find({ username: { $regex: `^${username}` } }).toArray();
        res.status(200).send(result);
    }
    catch (err) {
        res.status(500).send(err);
